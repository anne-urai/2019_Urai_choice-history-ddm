...
tv=1:timest;
  ...
  TH1=max(y,thr/2);
  TH2=min(thr/2,-y+thr);
 I=[(inp)*dt+randn(iters,150)*noise_std*sqrt(dt)+(spbias.*stim).*dt randn(iters,300)*noise_std*sqrt(dt)+spbias.*stim.*dt]; 

P=ones(iters,1)*thr/2+(spbias2.*stim)+(-1+2*rand(iters,1))*bsp;
