...
for d = 1:length(datasets),
    ...
    colors = linspecer(9, 'sequential'); % red blue green
    ...
    switch d
        case {1 3} % only 1 coherence level
    ...
    xlabel('dc ~ previous error', 'color', bestcolor(2, :));
    ylabel('dc ~ previous correct', 'color', bestcolor(3, :));
    
    subplot(4,4,5); hold on;
    scatterHistDiff(results.z_preverror, results.z_prevcorrect, [], [], colors(5, :));
    xlabel('z ~ previous error', 'color', bestcolor(2, :));
    ylabel('z ~ previous correct', 'color', bestcolor(3, :));
