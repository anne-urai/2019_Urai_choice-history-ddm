...
if ~isnumeric(sj),
    ...
    % exclude P16, 11 and 37 (and 31 because no MRI!)
    ...
    try
        % serial choice bias groups - based on HDDM vbias, run over all sessions
        tab = readtable(sprintf('%s/CSV/allindividualresults_megsessions.csv', subjectdata.path));
        tab = tab(tab.session == 0, :);
        tab(~ismember(tab.subjnr, subjectdata.all), :) = [];
        
        subjectdata.alternators = tab.subjnr(tab.repetition < 0.5)';
        subjectdata.repeaters   = tab.subjnr(tab.repetition >= 0.5)';
        assert(isequal(sort(subjectdata.all), sort([subjectdata.alternators subjectdata.repeaters])), 'subject nr mismatch');
    end
    
    % who shows a proper gamma-band response? defined in B2d
    subjectdata.gammaplus   = [4,6,7,8,10,12,13,14,15,18,20,21,22,23,25,28,29,34,35,38,39,40,41,43,46,47,48,50,51,52,55,56,57,59,60,61,62,64];
    subjectdata.gammaminus  = [2,3,5,9,17,19,24,26,27,30,31,32,33,36,42,44,45,49,53,54,58,63,65];
