...
models = {'data', 'stimcoding_dc_z_prevresp', 'stimcoding_dc_z_prevresp_multiplicative'};
% BLACK FOR DATA, BLUE FOR DC_Z, CYAN FOR MULTIPLICATIVE
cyan = cbrewer('seq', 'GnBu', 10);
thesecolors = {[0 0 0], mean(colors([1 2], :)), cyan(5, :)};
...
for d = [4 5]
    for m = 1:length(models),
        ...
        mat_unpref  = mat{mat.biased == -1, 3:end};
        ...
        switch models{m}
            case 'data'
                errorbar(1:length(unique(abs(alldata.stimulus))), nanmean(mat_biased), ...
                    1.96* nanstd(mat_biased) ./ sqrt(size(mat_biased, 1)), ...
