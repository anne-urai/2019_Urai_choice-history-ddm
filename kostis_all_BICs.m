...
model(4).name = {'d. Dynamic DDM', 'collapsing bounds'};
model(4).ticklabels = {'z', 'v_{bias}', 'z+v_{bias}'};
model(4).colors = [colors; mean(colors([1 2], :))];
model(4).basevalue = 0;
model(4).ylim = [-5000 500];

model(5).data = [results.ouD_input_bic results.ouD_lambda_bic];
model(5).name = {'e. Ornstein-Uhlenbeck' ''};
model(5).ticklabels = {'input bias', '\lambda bias'};
model(5).colors = [cols2([3 4], :)];
model(5).basevalue = 0;
model(5).ylim = [-3000 300];

model(6).data = [results.ouDColl_input_bic results.ouDColl_lambda_bic];
model(6).name = {'f. Ornstein-Uhlenbeck' 'collapsing bounds'};
model(6).ticklabels = {'input bias', '\lambda bias'};
model(6).colors = [cols2([3 4], :)];
model(6).basevalue = 0;
